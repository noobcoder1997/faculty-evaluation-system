<?php
	...
	$resultIDs =mysqli_query($mysqli, "SELECT * from supervisor WHERE row='$rowSP'");
...
<div class="container">
	<div class="row">
		...
		<div class="col-md-3">
			...
			<div class="col-md-12 label-warning preClass">
				...
				Semester: <?php echo $se;?> &nbsp;&nbsp; <span data-toggle='modal' data-target='#prefe3' style='color: #5a8dee;'><span class='glyphicon glyphicon-edit'></span></span>
			</div>
		</div><br/>
	</div>
	<br/>
	<div class="row">
		<?php
        $frow = $rs['fac_no'];
		$resEvaluateRecordU =mysqli_query($mysqli, "SELECT * from faculty where dept = '$department' and row <> '$frow' order by ln asc");
		while($rErs = mysqli_fetch_assoc($resEvaluateRecordU)){
            if('All' == $ay1 && 'All' == $sem1)
            ...
            elseif('All' == $sem1)
                $resEvaluateRecord =mysqli_query($mysqli, "SELECT * from schedule where stud_no = '$row' and  ay = '$ay1' order by ay desc, sem desc");
			     ...
			     $resEvaluateRecord =mysqli_query($mysqli, "SELECT * from schedule where ay = '$ay1' and  sem = '$sem1' and fac_no = '".$rErs['row']."'");
			if ($rER = mysqli_fetch_assoc($resEvaluateRecord)){	
				...
				if ($sERs = mysqli_fetch_assoc($searchEvaluateRecord)){
					echo "<div class='col-md-4'>";
						echo "<a type='button' class='btn btn-success btn-block' style='text-align: left;' data-toggle='modal' data-target='#evaluatePage".$rER['row']."'>";
									$resEvRecord =mysqli_query($mysqli, "SELECT * from faculty where row = '".$rER['fac_no']."'");
									if ($rERs = mysqli_fetch_assoc($resEvRecord)){
										echo "&nbsp&nbsp<strong>".strtoupper($rERs['fn']." ".$rERs['ln']);
										echo "<center><h2><span class='glyphicon glyphicon-check'></span> Evaluated</strong></h2> ".$rER['ay']."-".($rER['ay']+1)." $rER[sem] </center>";
									}
						echo "</a><br/>";
					echo "</div>";
				}else{
					echo "<div class='col-md-4'>";
						echo "<a type='button' class='btn btn-primary btn-block' style='text-align: left;' data-toggle='modal' data-target='#evaluatemodal".$rER['row']."' onclick='warning();'>";
									$resEvRecord =mysqli_query($mysqli, "SELECT * from faculty where row = '".$rER['fac_no']."'");
									if ($rERs = mysqli_fetch_assoc($resEvRecord)){
										echo "&nbsp&nbsp<strong>".strtoupper($rERs['fn']." ".$rERs['ln']);
										echo "<center><h2><span class='glyphicon glyphicon-edit'></span> Evaluate</strong></h2> ".$rER['ay']."-".($rER['ay']+1)." $rER[sem] </center>";
									}
						echo "</a><br/>";
					echo "</div>";
				}
							...
							<script>
								function subEval<?php echo $rER['row'];?>(){
									<?php
										...
										echo "var datas = '';";
										echo "
											datas = ".$datas."+'&eval=1'+'&evaluator_no=".$row."'+'&fac_no=".$rER['fac_no']."'+'&comment='+comment".$rER['row'].";
											$.ajax({
												type: 'POST',
...
<div class="modal fade" id="prefe3" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			...
			<div class="modal-body">
				<div class="form-group">
					<label for="aySched3">School Year</label>
					<select class="form-control" id="aySched3">
						<?php
				...
				<div class="form-group">
					<label for="semSched3">Semester</label>
					<select class="form-control" id="semSched3">
